...
def m():
    ...
    missing_urls = ["https://findthatpostcode.uk/postcodes/M1A%201AM.json"]
...
def test_check_columns_exist():
    ...
    df = pd.DataFrame([{v: k for k, v in enumerate(mandatory_columns)}])
    cache = DummyCache()
    stage = CheckColumnsExist(df, cache, None)
    result_df = stage.run()

    for m in mandatory_columns:
        df = pd.DataFrame([{v: k for k, v in enumerate(mandatory_columns) if v != m}])
        stage = CheckColumnsExist(df, cache, None)
...
def test_company_lookup(m):
    ...
    cache = DummyCache()
    cache["company"] = {}
    cache["charity"] = {"GB-COH-00198344": {"test_data": "test_value"}}
    stage = LookupCompanyDetails(df, cache, None)
    result_df = stage.run()
    assert len(cache["company"]) == 1
    assert (
        json.loads(cache["company"]["GB-COH-04325234"])["primaryTopic"]["CompanyName"]
        == "CANCER RESEARCH UK"
    )
...
def test_postcode_lookup(m):
    ...
    assert (
        json.loads(cache["postcode"]["L4 0TH"])["data"]["attributes"]["laua_name"]
        == "Liverpool"
    )
    ...
    df = pd.DataFrame(
        {
            ...
            "__org_age": pd.to_timedelta(
                [0 * 365, 10 * 365, 6 * 365, 150 * 365], unit="D"
            ),
        }
    )
    cache = DummyCache()
