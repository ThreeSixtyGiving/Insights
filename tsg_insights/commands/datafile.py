...
@click.option(
    "--file-limit", default=None, type=int, help="maximum file size to import"
)
@with_appcontext
    ...
    for publisher, files in list(reg.items()):
        ...
        for file_ in files:
            ...
            if file_["file_size"] < file_limit:
                try:
                    fileid, filename, headers = get_dataframe_from_url(
                        file_["download_url"]
                    )
...
def cli_remove_all_files():
    click.confirm(
        "This will delete all cached data. Are you sure you want to continue?",
        abort=True,
    )
    cache = get_cache()
