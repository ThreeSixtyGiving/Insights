...
from .blueprints import cache, data, fetch, home, job
from .commands import additional_data, datafile, registry, worker
...
def create_app(test_config=None):
    ...
    app.config.from_mapping(
        ...
        FILE_CACHE=os.environ.get(
            "FILE_CACHE", "filesystem"
        ),  # use 'redis' or 'filesystem'
        # Newsletter
        ...
        ],  # domain names allowed to fetch data from
    )
    ...
    app.config["REDIS_URL"] = os.environ.get(
        app.config["REDIS_ENV_VAR"], app.config["REDIS_DEFAULT_URL"]
    )
    ...
    def inject_template_scope():
        ...
        injections.update(
            cookies_consented=cookies_consented, cookies_asked=cookies_asked
        )
