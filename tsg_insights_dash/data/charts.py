...
DEFAULT_LAYOUT = {
    "font": {"family": 'Roboto, "Source Sans Pro", sans-serif;', "size": 18},
    "yaxis": {
        "visible": False,
        "showgrid": False,
        "showline": False,
...
DEFAULT_CONFIG = {
    ...
    "modeBarButtons": [["toImage", "sendDataToCloud"]],
    "scrollZoom": False,
}
...
def chart_title(title, subtitle=None, description=None):
    return html.Figcaption(
        ...
        children=[
            ...
            (
                html.P(
                    className="results-page__body__section-subtitle", children=subtitle
                )
                if subtitle
                else None
            ),
...
def chart_n(count, label="grant"):
    return html.Div(
        ...
        children="Based on {:,.0f} {}.".format(count, pluralize(label, count)),
    )


def message_box(title, contents, error=False):
    if isinstance(contents, str):
        contents_div = dcc.Markdown(className="", children=contents)
    else:
        contents_div = (html.P(className="", children=contents),)
    ...
    return html.Div(
        ...
        children=[
            html.H2(className="results-page__body__section-title", children=title),
            html.Div(
                className="results-page__body__section-note", children=contents_div
            ),
        ],
    )
...
def get_bar_data(values, name="Grants", chart_type="bar", colour=0):
    ...
    titles = [
        " - ".join(get_unique_list(i)) if isinstance(i, (list, tuple)) else i
        for i in titles
    ]
    bar_data = {
        "x": titles,
        ...
        "type": chart_type,
        "name": name,
        "marker": {"color": THREESIXTY_COLOURS[colour % len(THREESIXTY_COLOURS)]},
        "fill": "tozeroy",
    }
    ...
    if len(data) <= 1:
        ...
        return chart_wrapper(
            html.Div(
                className="results-page__body__section-note",
                children=[
                    "{:,.0f} funders included in results. Top 10 funders: ".format(
                        len(data)
                    ),
...
def grant_programme_chart(df):
    ...
    if (
        "Grant Programme:0:Title" not in df.columns
        or len(df["Grant Programme:0:Title"].unique()) <= 1
    ):
        return
...
def amount_awarded_chart(df):
    ...
    currencies = list_to_string([get_currency_name(c) for c in data.columns.tolist()])
    ...
    if len(data.columns) > 1:
        ...
        bars = [
            get_bar_data(
                series[1],
                name="{} ({} grants)".format(
                    get_currency_name(series[0]), series[1].sum()
                ),
                colour=colours.get(series[0], k + 3),
            )
            for k, series in enumerate(data.iteritems())
    ...
    return chart_wrapper(
        dcc.Graph(
            ...
            figure={"data": bars, "layout": DEFAULT_LAYOUT},
            config=DEFAULT_CONFIG,
        ),
        chart["title"],
        subtitle=units,
...
def org_identifier_chart(df):
    ...
    return chart_wrapper(
        dcc.Graph(
            ...
            figure={"data": [get_bar_data(data)], "layout": DEFAULT_LAYOUT},
            config=DEFAULT_CONFIG,
        ),
        chart["title"],
        subtitle=chart.get("units"),
...
def awards_over_time_chart(df):
    ...
    if df["Award Date"].max().strftime("%Y-%m") == df["Award Date"].min().strftime(
        "%Y-%m"
    ):
        return message_box(
            "Award Date",
            "All grants were awarded in {}.".format(
                df["Award Date"].min().strftime("%B %Y")
            ),
            error=False,
        )
    ...
    xbins_sizes = (("M1", "by month"), ("M3", "by quarter"), ("M12", "by year"))
...
def region_and_country_chart(df):
    ...
    if (
        ...
        or (df["__geo_ctry"].count() + df["__geo_rgn"].count()) == 0
    ):
        return message_box(chart["title"], chart.get("missing"), error=True)
    ...
    return chart_wrapper(
        dcc.Graph(
            ...
            figure={
                "data": [
                    get_bar_data(
                        data["Grants"].iloc[::-1], chart_type="column", colour=2
                    )
                ],
                "layout": layout,
            },
...
def organisation_type_chart(df):
    ...
    subtitle = chart.get("units")
    description = html.P(
        """Organisation type is based on official organisation identifiers,
    ...
    if len(data) > 10:
        ...
        children = [
            html.P(
                "The following types are also found in the data:",
                className="results-page__body__section-note",
            ),
            ...
            figure={
                "data": [
                    go.Pie(
                        ...
                        hole=0.4,
                        marker={"colors": THREESIXTY_COLOURS},
...
def organisation_income_chart(df):
    ...
    if (
        "__org_latest_income_bands" not in df.columns
        or df["__org_latest_income_bands"].count() == 0
    ):
        return message_box(chart["title"], chart.get("missing"), error=True)
    ...
    return chart_wrapper(
        dcc.Graph(
            ...
            figure={"data": [get_bar_data(data, colour=3)], "layout": DEFAULT_LAYOUT},
            config=DEFAULT_CONFIG,
        ),
        chart["title"],
        subtitle=chart.get("units"),
...
def imd_chart(df):
    ...
    return chart_wrapper(
        dcc.Graph(
            id="imd_chart",
            figure={"data": [get_bar_data(data)], "layout": layout},
            config=DEFAULT_CONFIG,
        ),
        chart["title"],
        subtitle=chart.get("units"),
...
def location_map(df, mapbox_access_token=None, mapbox_style=None):
    ...
    data.append(
        go.Scattermapbox(
            ...
            hoverinfo="text",
            text=geo.apply(
                lambda row: "{} ({} grants)".format(row[popup_col], row["grants"])
                if row["grants"] > 1
                else row[popup_col],
                axis=1,
            ).values,
    ...
    layout = go.Layout(
        ...
        mapbox=dict(
            ...
            bearing=0,
            center=dict(lat=54.093409, lon=-2.89479),
            pitch=0,
            zoom=5,
            style=mapbox_style,
        ),
    ...
    if other_currencies:
        others.extend(
            [
                html.P(
                    [
                        "These results include grants in {} currencies. ".format(
                            len(c)
                        ),
                        "The amounts above refer to {:,.0f} grants in {}. ".format(
                            main_currency["grants"], get_currency_name(c[0][0])
                        ),
                        html.Br(),
                        "There is also ",
                        list_to_string(
        ...
        html.Div(
            ...
            children=[
                html.Div(
                    ...
                    children=[
                        html.P(
                            className="", children="{:,.0f}".format(stats["grants"])
                        ),
                        html.H4(
                            className="", children=pluralize("grant", stats["grants"])
                        ),
                    ],
                ),
                html.Div(
                    ...
                    children=[
                        html.P(
                            className="", children="{:,.0f}".format(stats["recipients"])
                        ),
                        html.H4(
                            className="",
                            children=pluralize("recipient", stats["recipients"]),
                        ),
                    ],
                ),
            ]
            + [
                html.Div(
                    ...
                    children=[
                        ...
                        html.H4(className="", children=main_currency["total_f"][1]),
            ...
            + [
                html.Div(
                    className="results-page__body__content__sphere",
                    style={"backgroundColor": THREESIXTY_COLOURS[2]},
                    children=[
                        ...
                        html.H4(
                            ...
                            children="(Average* grant)",
                        ),
                    ],
                )
            ],
        ),
        html.Div(className="results-page__body__section-attribution", children=others),
    ]
...
def get_funder_output(df, grant_programme=[]):
    ...
    if len(funder_names) > 5:
        funders = html.Span(
            "{:,.0f} funders".format(len(funder_names)), className=funder_class
        )
        subtitle = [
            html.Div(className="mt2 gray f4", children=list_to_string(funder_names))
        ]
    else:
        funders = list_to_string(
            [html.Span(f, className=funder_class) for f in funder_names], as_list=True
        )
...
def get_file_output(metadata):
    ...
    if metadata.get("registry_entry"):
        output.append(html.P(html.Strong("From the 360Giving data registry")))
        output.append(
            html.P(
                [
                    "Published by ",
                    html.A(
                        metadata.get("registry_entry", {})
                        .get("publisher", {})
                        .get("name"),
                        href=metadata.get("registry_entry", {})
                        .get("publisher", {})
                        .get("website"),
                        target="_blank",
                    ),
                    ...
                    html.A(
                        ...
                        href=metadata.get("registry_entry", {}).get("license"),
                        target="_blank",
                    ),
                    " licence.",
                ]
        ...
        output.append(
            html.P(
                ...
                + [
                    " (",
                    metadata.get("registry_entry", {})
                    .get("datagetter_metadata", {})
                    .get("file_type"),
                    ")",
                ]
                if metadata.get("registry_entry", {}).get("datagetter_metadata")
                else []
            )
        )
