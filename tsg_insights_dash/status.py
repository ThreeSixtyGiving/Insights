...
layout = html.Div(
    ...
    children=[
        html.Div(
            className="fl w-25-l w-100 pa2-l",
            children=[
                message_box(
                    title="Filter data",
                    contents=[
                            ...
                            children=[
                                ...
                                html.Div(
                                    ...
                                    children=[
                                        html.Label("Licence"),
                                        dcc.Dropdown(
                                            id="status-licence", multi=True, options=[]
                                        ),
                                    ],
                                ),
                                html.Div(
                                    ...
                                    children=[
                                        html.Label("Currency"),
                                        dcc.Dropdown(
                                            id="status-currency", multi=True, options=[]
                                        ),
                                    ],
                                ),
                                html.Div(
        ...
        html.Div(
            className="fl w-75-l w-100 pa2-l",
            children=[
...
@app.callback(
    Output("status-licence", "options"), [Input("status-container", "children")]
)
def get_status_options(_):
    ...
    return [{"label": v, "value": k} for k, v in licenses.items()]


@app.callback(
    Output("status-currency", "options"), [Input("status-container", "children")]
)
def get_currency_options(_):
    ...
    return [
        {"label": "{} [{}]".format(babel.numbers.get_currency_name(c), c), "value": c}
        for c in currencies
    ]


@app.callback(
    Output("status-file-type", "options"), [Input("status-container", "children")]
)
def get_filetype_options(_):
    ...
    return [
        {"label": "{} ({})".format(v[0], v[1]), "value": k}
        for k, v in filetypes.items()
    ]


@app.callback(
    Output("status-rows", "children"),
    [
    ...
    for pub, pub_reg in reg.items():
        rows.append(
            html.Div(
                ...
                children=[
                    html.Div(
                        className="w-100 cf pa3",
                        children=[
                            html.A(
                                className="f3 link black b",
                                href=pub_reg[0].get("publisher", {}).get("website"),
                                target="_blank",
                                children=[pub_reg[0].get("publisher", {}).get("name")],
                            ),
                            html.Img(
                                className="fr mw5",
                    ...
                    html.Div(
                        ...
                        children=[
                            ...
                            html.Div(
                                ...
                                children=[file_row(v, len(pub_reg)) for v in pub_reg],
                            ),
                        ],
                    ),
                ],
...
def file_row(v, files=1):
    ...
    validity = {
        "valid": {
            "class": "",
            "icon": None,
            "messages": {"positive": "Valid data", "negative": "Data invalid"},
        ...
        "downloads": {
            ...
            "icon": None,
            "messages": {
                "positive": "Download link working",
                "negative": "Download link broken",
            },
        ...
        "acceptable_license": {
            ...
            "icon": None,
            "messages": {
                "positive": "Licensed for reuse",
                "negative": "Licence doesn't allow reuse",
            },
    ...
    return html.Div(
        className="bt pa3 b--black-10",
        children=[
            html.Div(
                className="content",
                children=[
                    html.A(
                        children=get_license_badge(
                            v.get("license"), v.get("license_name")
                        ),
                        href=v.get("license"),
                        target="_blank",
                        className="fr",
                    ...
                    html.Div(
                        ...
                        children=[
                            ...
                            html.Span(
                                ", ".join(
                                    [
                                        babel.numbers.get_currency_name(k)
                                        for k in v.get("datagetter_aggregates", {}).get(
                                            "currencies", {}
                                        )
                                    ]
                                )
                            ...
                            html.Span(
                                [
                                    "Last modified ",
                                    html.Time(
                                        dateTime=v.get("modified"),
                                        children=[
                                            humanize.naturaldelta(
                                                datetime.datetime.now()
                                                - dateutil.parser.parse(
                                                    v.get("modified"), ignoretz=True
                                                )
                                            )
                                        ],
                                    ),
                    ...
                    html.Div(
                        ...
                        children=[
                            html.Div(
                                className="flex",
                                children=get_file_stats(v, as_statistic=True),
                            )
                        ],
                    ),
                ],
            ...
            html.Div(
                ...
                children=[
                    html.Span(
                        [validity[i]["icon"], validity[i]["message"]],
                        className=validity[i]["class"] + " mr3",
                        style={"color": validity[i].get("color")},
                    )
                    for i in validity
                ],
            ),
...
def get_file_stats(v, separator=None, as_statistic=True):
    ...
    stats.append(to_statistic(humanize.intcomma(count), pluralize("grant", count)))
...
def get_date_range(v):
    ...
    max_award_date = datetime.datetime.strptime(
        agg.get("max_award_date", None), "%Y-%m-%d"
    )
    min_award_date = datetime.datetime.strptime(
        agg.get("min_award_date", None), "%Y-%m-%d"
    )
...
def get_license_badge(url, name):
    if "creativecommons.org/licenses" in url:
        ...
        return [
            html.Img(
                src="https://mirrors.creativecommons.org/presskit/icons/{}.png".format(
                    c.lower()
                ),
                title=name,
    ...
    if "creativecommons.org" in url:
        badge_url = url.replace(
            "creativecommons.org/licenses", "i.creativecommons.org/l"
        )
        badge_url = badge_url.replace(
            "creativecommons.org/publicdomain", "i.creativecommons.org/p"
        )
